document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('generate-resume-btn').addEventListener('click', () => {
        ...
        if (data.name && data.email) { saveUserData(data); }
    ...
    async function saveUserData(data) {
        try {
            ...
            if (response.ok) { console.log('User data saved.'); } else { console.error('Failed to save user data.'); }
    ...
    function generatePreview(data) {
        ...
        const resumeHTML = `
            <div id="resume-paper" style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 10.5pt; line-height: 1.6; color: #333;">
                <style>#resume-paper a{text-decoration:none;color:inherit}#resume-paper h2{font-family:'Trebuchet MS',sans-serif;font-size:14pt;color:#2c3e50;font-weight:bold;margin:25px 0 10px;padding-bottom:5px;border-bottom:1.5px solid #ccc}#resume-paper h3{font-size:11pt;font-weight:bold;margin:15px 0 2px}#resume-paper p{margin-top:0;margin-bottom:10px}#resume-paper .skills-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:20px}#resume-paper .skills-grid h3{margin-top:0}</style>
                <header style="display:flex;width:100%;align-items:stretch"><div style="background-color:#2c3e50;padding:20px;display:flex;align-items:center;justify-content:center;min-width:180px">${data.profilePic ? `<img src="${data.profilePic}" alt="Profile" style="width:130px;height:130px;border-radius:50%;object-fit:cover">` : ""}</div><div style="background-color:#ffc107;color:#2c3e50;padding:25px 30px;flex-grow:1;display:flex;flex-direction:column;justify-content:center"><h1 style="font-family:'Arial Black',Gadget,sans-serif;font-size:38pt;text-transform:uppercase;margin:0;line-height:1.1">${data.name}</h1><p style="font-size:14pt;margin:5px 0 15px">${data.role}</p><div style="font-size:10pt;display:flex;flex-direction:column;align-items:flex-start;gap:5px"><a href="mailto:${data.email}" style="display:flex;align-items:center">${emailIcon} ${data.email}</a><a href="${data.linkedin}" target="_blank" style="display:flex;align-items:center">${linkedinIcon} ${data.linkedin}</a><a href="${data.github}" target="_blank" style="display:flex;align-items:center">${githubIcon} ${data.github}</a></div></div></header>
                ...
                <main style="padding:0 25px 25px"><section><h2>I. Career Objective</h2><p>${data.summary}</p></section><section><h2>II. Work Experience</h2>${data.experience.map(e => `<div><h3>${e.title}</h3><p style="margin-top:0;margin-bottom:5px"><em>${e.company}</em></p><p>${e.desc}</p></div>`).join("")}</section><section><h2>III. Projects</h2>${data.projects.map(p => `<div><h3>${p.title}</h3><p>${p.desc}</p></div>`).join("")}</section><section><h2>IV. Technical Skills</h2><div class="skills-grid"><div><h3>Programming Languages</h3><p>${data.programmingSkills.replace(/\n/g, "<br>")}</p></div><div><h3>Web Technologies</h3><p>${data.webSkills.replace(/\n/g, "<br>")}</p></div><div><h3>Soft Skills</h3><p>${data.softSkills.replace(/\n/g, "<br>")}</p></div></div></section><section><h2>V. Education</h2>${data.education.map(e => `<div><h3>${e.school}</h3><p>${e.degree} in ${e.field} - ${e.year}</p></div>`).join("")}</section></main>
            </div>`;
    ...
    // --- FIXED: DOWNLOAD AND SHARE BUTTONS WITH HIGH QUALITY SETTINGS ---
    document.getElementById('download-pdf-btn').addEventListener('click', () => {
        const resumeElement = document.getElementById('resume-paper');
        const opt = {
            margin: 0,
            filename: 'Tanishk_Pal_Resume.pdf',
            image: { type: 'jpeg', quality: 1.0 }, // Use maximum quality JPEG
            html2canvas: {
                scale: 5, // Increase scale for ultra-high resolution
                useCORS: true
            },
    ...
    document.getElementById('download-jpeg-btn').addEventListener('click', () => {
        const resumeElement = document.getElementById('resume-paper');
        // Use a higher scale for a much clearer image
        html2canvas(resumeElement, { scale: 5, useCORS: true }).then(canvas => {
